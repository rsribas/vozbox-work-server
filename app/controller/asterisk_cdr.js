...
class AsteriskCdr extends Controller {
  async index() {
      ...
      ctx.validate(
        {
          ...
          sort: { type: 'string?', default: 'calldate-desc' },
        },
        ctx.request.body
      )
    } catch (e) {
    ...
    try {
      const result = await service.asteriskCdr.findAndCountAllByUid({
  ...
  async sumAmount() {
    ...
    try {
      const result = await service.asterisk_cdr.findSumPriceByDate(startDate, endDate)
      ctx.print = result
    } catch {
      ctx.print = { errorCode: 2 }
    }
  ...
  async amountGroup() {
    ...
    const result = await service.asterisk_cdr.findAmountGroup(startDate, endDate)
    ctx.print = result
  }
}
