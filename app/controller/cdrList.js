...
class CdrList extends Controller {
  async index() {
  ...
  async create() {
    ...
    try {
      const result = await service.cdrList.create({ content })
      ctx.print = {
        data: result,
        msg: '创建成功'
      }
  ...
  async update() {
    ...
    try {
      await service.cdrList.updateById(id, data)
      ctx.print = { msg: '更新成功' }
    } catch (e) {
      ctx.print = {
        errorCode: 400,
  ...
  async destroy() {
    const { ctx, service } = this
    const id = ctx.params.id
    await service.cdrList.deleteById(id)
    ctx.print = { msg: '删除成功' }
  }
}
